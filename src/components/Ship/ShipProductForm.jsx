...
import { shipProduct } from '../../SupplyChainContract';
import { FaTimes } from 'react-icons/fa';
import { useDispatch, useSelector } from 'react-redux';
import { globalActions } from '../../store/globalSlices';
import { toast } from 'react-toastify';

const ShipProductForm = () => {
  const [productId, setProductId] = useState('');
  const [distributorAddress, setDistributorAddress] = useState('');

  const { shipModal, buttonSubmit } = useSelector((states) => states.globalStates);
  const { setShipModal } = globalActions;
  const dispatch = useDispatch();

  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!productId || !distributorAddress) return;

    await toast.promise(
      new Promise(async (resolve, reject) => {
        await shipProduct(productId, distributorAddress)
          .then((tx) => {
            closeModal();
            resolve(tx);
          })
      ...
      {
        pending: 'Shipping Product...',
        success: 'Product shipped successfully 👌',
  ...
  return (
    <div
      ...
      transition-transform z-50 duration-300 ${shipModal}`}
    >
      <div
